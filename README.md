...
2. **AI 编译器教学全链路**：基于 MLIR/ONNX 的 P1–P14 pipeline，从模型解析到 GPU/量化/内存/分区教学模拟，覆盖 AI 编译器核心知识地图
...
这会构建 **P5（`5_stablehlo_opt`）及 P6–P14**（纯 C++17，无外部依赖）。未安装的可选依赖会在 cmake 阶段以 STATUS 提示自动跳过（如 `ANTLR4 not found`、`GTest not found`），不影响基础构建。
...
> 注：真实 MLIR StableHLO Pass 插件（Conv+BN Fusion，`mlir-opt` 加载）以及 PyTorch/torch-mlir → StableHLO 导出脚本已迁移至 [`mlir_pass`](../mlir_pass/) 仓库（`lib/Transforms/ConvBNFusion.cpp`、`tools/mlir-opt-plugin/`、`scripts/torch_export/`）。本仓库 `src/mlir/gpu/` 全部为纯 C++ 自定义 IR 教学实现，不再依赖 MLIR / StableHLO 运行库。

---

## AI 编译器 Pipeline 总览（`src/mlir/gpu/`）

P1–P14 教学 pipeline，从模型输入到 GPU/量化/内存/图分区知识地图：
...
| **L2** | Tensor Operator Layer / High-Level Math Layer（张量算子层 / 高级数学层） | StableHLO（文本）；P4、P5 |
| **L3** | Structured Op & Memory Layer（结构化算子与内存层） | Linalg on tensor → OSB → Linalg on memref；P6–P7 |
| **L4** | Kernel Loop & Vector Layer（内核循环与矢量层） | SCF / Affine / Vector → LLVM / GPU codegen；P8–P13 |
...
├─────────────────────────────────────────────────────────────────────────────┤
│  P4 / P5: L2 StableHLO 高级张量算子                                            │
│  ONNX→StableHLO (tier 1/2/3, 含 Softmax/Attention/RMSNorm)                    │
│  → StableHLO 6-Step 图优化（shlo_graph 自定义 IR）                            │
├─────────────────────────────────────────────────────────────────────────────┤
│  P6–P7: L3 Linalg 结构化算子与内存                                             │
│  Linalg Tiling & Fusion → One-Shot Bufferization                            │
├─────────────────────────────────────────────────────────────────────────────┤
│  P8–P10: L4 Kernel Loop / Vector / LLVM 后端 (纯 C++)                         │
│  SCF/Affine 循环优化 → Vector 向量化 → LLVM Backend (ISel/RegAlloc/Sched)     │
├─────────────────────────────────────────────────────────────────────────────┤
│  P11–P13: L4 GPU 映射 & 部署优化 (纯 C++)                                      │
...
| `12_quantization/` | **P12** | 量化 7-Step：校准统计→Scale 计算→量化融合→**真实 Q/DQ 图改写**→混合精度分析→Speedup 估算 | **无** | `run_quant` |
| `13_memory_planning/` | **P13** | 内存规划 8-Step：Liveness→干涉图→Offset 分配→Buffer 复用→In-place 优化→峰值分析→**KVCache decode 场景** | **无** | `run_memplan` |
| `14_graph_partition/` | **P14** | Graph Partitioning 教学：单层 Transformer Attention/FFN 切分、边界张量与通信量估算 | **无** | `run_graph_partition_demo` |
...
make run_quant              # P12: Quantization & 混合精度 pipeline（含真实 Q/DQ 图改写）
make run_memplan            # P13: Memory Planning & Buffer 复用 pipeline（含 KVCache decode 场景）
```
...
```bash
# 运行主要 MLIR 阶段（P1–P13 所有已构建的 target；P14 单独见 run_graph_partition_demo）
make run_mlir
...
| `make run_golden` | — | P4 | ONNX Runtime vs NumPy 数值对齐（18 项 tier-3 + decode/dynamic_mn/f16 + P12 Q/DQ） |
...
| `make run_mlir` | `DOMAIN=mlir` | P1–P13 | 依次运行所有 MLIR target；缺少可选依赖的 target 自动跳过 |
...
`gen_lowering_models.py` 生成的 tier 3 fixture 包括：…、`lowering_matmul_softmax.onnx`、`lowering_layout_conv.onnx`、`lowering_transformer_block.onnx`、`lowering_qdq_matmul.onnx` 等；P12 `quant_qdq_matmul.onnx` 经 P4 tier 3 导出。`run_golden` 用 ONNX Runtime 对比 NumPy reference 做数值对齐（**18 项**：15 项 lowering + decode_step/dynamic_mn/matmul_f16 + P12 Q/DQ）。
...
ninja -C build test_transformer_e2e   # Transformer 七件套 + GEMM/FFN/block 共 11 fixture
```
...
> 真实 MLIR C++ API 的 StableHLO Conv+BN Fusion Pass 插件已迁移至 [`mlir_pass`](../mlir_pass/) 仓库（`lib/Transforms/ConvBNFusion.cpp` + `tools/mlir-opt-plugin/`，`conv-bn-fusion` / `aicom-fusion` pipeline，且实现更完整——同时折叠 scale 与 bias）。PyTorch/torch-mlir → StableHLO 导出脚本见 `mlir_pass/scripts/torch_export/`。本仓库 P5 保留纯 C++ 教学版 `5_stablehlo_opt/`（在自定义 shlo_graph IR 上镜像同一 Conv+BN 变换）。
...
### P12: Quantization & Mixed Precision (`12_quantization/`)
...
|------|---------------|-----------|
| ONNX 解析 / mini IR / 教学模拟 | P1–P3、P6–P13 | — |
| ONNX → StableHLO lowering | P4 tier 1/2/3 | 消费导出的 `.mlir` |
| StableHLO 图优化 | P5 纯 C++ 模拟（`5_stablehlo_opt`） | 19 个 fusion pass + `AICompilerPlugin`（含真实 Conv+BN Fusion） |
| Linalg → LLVM → JIT | 概念教学 | `pipe-demo` 完整 pipeline |
| 数值对齐 golden test | `run_golden`（ONNX Runtime，18 项） | — |
| 跨仓库 Transformer/e2e | `run_lowering_l3 --mlir-only`（七件套 + GEMM/QDQ/layout/decode） | `test_transformer_e2e` + 39 LIT + 10 条 e2e |

推荐验证顺序：先 `cmake --build build --target gen_lowering_models gen_quant_models run_lowering run_golden run_calib_to_quant run_quant_qdq run_graph_partition_demo`，再在 `mlir_pass` 执行 `ninja -C build test_transformer_e2e test_quant_e2e test_dynamic_e2e test_kvcache_e2e test_jit_golden test_layout_e2e test_torch_e2e test_broadcast_e2e test_partition_smoke test_lit_filecheck test_mlir_opt_plugin`。
