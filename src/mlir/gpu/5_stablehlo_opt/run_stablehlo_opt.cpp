// run_stablehlo_opt.cpp — P5 (5_stablehlo_opt): StableHLO graph optimization (shlo_graph)
//
...
// Mirrors the real MLIR Conv+BN fusion (see mlir_pass ConvBNFusion.cpp), but on our graph IR
...
static Graph make_test_comprehensive() {
  ...
  // Conv + BN Fusion (mirrors mlir_pass ConvBNFusion.cpp Conv+BN pattern)
