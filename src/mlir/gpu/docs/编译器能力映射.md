...
| 算子数值对齐 | **JIT 数值校验** `test_jit_golden` **6** 项（编译产物 vs NumPy）；shell 另有 JIT smoke | **参考实现校验** `run_golden` **18** 项（ORT vs NumPy，不跑编译器） | **强契合**（两套测试互补，勿混为一谈；见学习主文档 §1.4） |
...
| 算子数值对齐 | `test_jit_golden`（JIT vs NumPy）+ shell JIT smoke | `run_lowering_golden.py`（ORT vs NumPy，**18** 项） | 两仓库 **强契合**（对象不同：编译产物 vs 参考实现） |
...
#### 7.1 参考实现校验（`run_golden`，ORT vs NumPy） ✅
...
| **验证** | `cmake --build build --target run_golden`（18/18 PASS） |
...
| 测试 | 「`mlir_pass` Shell **36** 项 + LIT **39** 项 + **10** 条跨仓库 e2e + JIT 数值校验 **6** 项；`mlir_compiler` 参考实现校验 `run_golden` **18** 项 + `run_calib_to_quant` + CI。工业级缺口见 **§2.2**。」 |
...
| JIT 数值校验 | `test_jit_golden`（编译产物 vs NumPy） | 强契合（与 `run_golden` 互补，对象不同） |
