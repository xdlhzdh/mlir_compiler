...
| **项目二** | [mlir_compiler](https://github.com/xdlhzdh/mlir_compiler) | 传统编译器 + AI 编译器全流程教学 | P1/P4/P5a 偏真实；P6–P12/P13 为教学级 `*_ir.h` 模拟 |
...
| **A5** FFN JIT golden | **已补强** | `test_jit_golden` **6 项**（含 gelu/swiglu P4） | sigmoid 快速路径 + erf 全链路 |
...
| TP/PP + NCCL | 多卡 collective runtime | P13 编译期切分 + comm bytes | 「多卡分布式 runtime」 |
...
| Paged KV / continuous batching | PagedAttention runtime | P12 静态 plan 概念 | 「PagedAttention runtime」 |
...
| 2 | **双重验证** | 至少两种：golden / LIT / shell / 跨仓库 e2e / JIT golden |
...
| PyTorch / FX / Dynamo | 无 | 仅 `4_torch_to_stablehlo/conv_bn_model.py` 导出 Conv+BN 小模型 | **缺口** |
...
| 自定义 Pass（TableGen） | **28** 个：[`include/AICompiler/Passes.td`](../../../../../../mlir_pass/include/AICompiler/Passes.td) | P5a 1 个 | pass **强契合** |
| Dialect Conversion / Lowering | 官方 `convert-*` + `chlo-legalize-to-stablehlo` + `stablehlo-legalize-to-linalg` | P4 tier 3 教学框架 [`conversion_framework.h`](../5_onnx_to_stablehlo/conversion_framework.h) | pass **强契合**；compiler **概念契合** |
...
| 模板要求 | mlir_pass | mlir_compiler | 等级 |
|----------|-----------|---------------|------|
| Conv+Bias+ReLU 融合 | `conv-bn-fusion` + `conv-bn-relu-fusion` | P3 mini_ir + P5a MLIR + P5b 模拟 | **强契合** |
| numpy-style Broadcast | `broadcast-simplify` + `test_broadcast_e2e` + golden | P4 tier 2 `lowering_broadcast.onnx` | **强契合** |
| 常量折叠 | `stablehlo-constant-fold` + `custom-linalg-opt` + LIT | P3 / P5b 模拟 | **强契合** |
...
| 多路径 lowering | `scf-seq` / `scf-par` / `affine` / `vector` + JIT | P8–P9 概念教学 | pass **强契合** |
| 布局转换 NCHW↔NHWC | `layout-bridge-legalize` 标注 + `test_layout_e2e` | P5b layout fold + `lowering_layout_conv.onnx` | **部分契合** |
| DCE / CSE | 官方 canonicalize / CSE（linalg/bufferize 阶段）+ `canonicalize_cse_smoke` LIT | P5b 模拟 | pass **强契合**；compiler **概念契合** |
...
| Q/DQ MatMul | `qdq-legalize` + `test_quant_e2e` **三条路径** | P4 + P11 + golden | **强契合** |
...
| KVCache | `kvcache-legalize` 标注 + LIT | P12 Stage 7 [`run_memory_planning.cpp`](../15_memory_planning/run_memory_planning.cpp) | **部分契合**（compiler 概念 + pass 标注） |
...
| 变长输入内存规划 | 无 | P12 ResNet + KVCache 场景 | compiler **概念契合** |

### F. 量化前端

| 模板要求 | mlir_pass | mlir_compiler | 等级 |
|----------|-----------|---------------|------|
| 校准 / Scale 计算 | 无 | P11 Stage 1–2 [`14_quantization/`](../14_quantization/) | compiler **概念契合** |
| Q/DQ 图改写 | `test_quant_e2e` 三条路径 | P11 Stage 4 + P4 Q/DQ lowering + golden | **强契合** |
| ONNX-QDQ 模型导入 | `test_quant_e2e` P11 链路 | `gen_quant_models.py` + P4 tier 3 | **强契合** |
| QAT / FP8 | 无 | 无 | **缺口** |
| 混合精度分析 | 无 | P11 Stage 5 敏感度表 | compiler **概念契合** |
...
| 算子数值对齐（golden） | `test_jit_golden` **6 项** + shell JIT | `run_golden` **18 项** | **强契合**（双仓库互补：ORT lowering + JIT 后端） |
...
| 模板要求 | mlir_pass | mlir_compiler | 等级 |
|----------|-----------|---------------|------|
| Graph Partitioning | `graph_partition_smoke.mlir` + `test_partition_smoke` | P13 `16_graph_partition/` Attention/FFN 切分教学 | **部分契合** |
| Tensor / Pipeline Parallel | 无 | 无 | **缺口** |
| KVCache 编译期优化 | 无 | P12 Stage 7 模拟 | compiler **概念契合** |
| 首字延迟 / 吞吐量数据 | 无 | 无 | **缺口** |
...
| MLIR 前端 Dialect + Lowering | Pass + StableHLO lowering | mlir_pass + P4/P5a | **可写**（不说 Dynamo） |
| Conv+Bias+ReLU 等图优化 Pass | conv-bn / conv-bn-relu fusion | mlir_pass + P3/P5a | **可写** |
| 动态 Shape + 维度推导 | P4 dynamic + P1 shape infer | mlir_compiler | **可写**（注明教学/有限算子） |
| TVM Relay 算子适配 | 无 | — | **不可写** |
| ONNX-QDQ 量化前端 | P11 Q/DQ 图改写 | mlir_compiler | **可写**（注明教学 IR） |
| 算子对齐测试框架 | LIT + run_golden + test_jit_golden + e2e | mlir_pass + P4 | **可写** |
...
| Graph Partitioning / 多卡 | P13 Attention/FFN 切分教学 | 无 runtime / NCCL |
| KVCache 编译期内存分析 | P12 Stage 7 | 模拟级，无 runtime |
...
|----------|-----------|---------------|------|
| StableHLO / XLA 生态 | 入口 dialect + fusion pass | P4 text IR + P5b 优化链 | pass 更「真」 |
...
| LLVM JIT 数值验证 | `pipe-demo --jit` | 无 | pass 独有 |
| torch-mlir 导出 | 无 | `conv_bn_model.py` 单模型 | 入门级 |
| LLM decode / paged KV | 无 | P12 KVCache 规划 | 概念级 |
...
| 算子 golden 对齐 | `test_jit_golden` + shell JIT | `run_lowering_golden.py`（**18** 项） | 两仓库 **强契合**（ORT lowering + JIT 互补） |
| CHLO→StableHLO 桥接 | linalg stage 内置 `chlo-legalize-to-stablehlo` | P4 GELU 导出 `chlo.erf` | **强契合**（教学管线） |
| 异构 / Memory Hierarchy | JIT + buffer opt | P12 + 早期 DSP/FPGA 项目 | 可关联叙事 |
...
StableHLO 优化 P5b             → fusion / linalg / bufferize 真实 pass
Linalg / Bufferize P6–P7       → 真实 one-shot-bufferize
SCF / Affine / Vector P8–P9    → 四条 --loop-mode 路径
量化 / KVCache P11–P12         → 前端图语义理解（规划/改写）
                               → JIT 证明数值正确
```

**推荐面试句：**

> 「`mlir_compiler` 让我按 P1–P12 把 AI 编译器全链路走通；`mlir_pass` 是我在真实 MLIR 上把 StableHLO 子图降到 LLVM 并 JIT 跑通的工程化实现。」
...
#### 7.1 ONNX Runtime 数值对齐（golden test） ✅

| 项 | 内容 |
|----|------|
| **状态** | 已实现：`run_lowering_golden.py` + CMake target `run_golden` |
| **覆盖** | `lowering_basic` / `softmax` / `attention` / `rmsnorm` / `rope` |
| **验证** | `cmake --build build --target run_golden`（5/5 PASS） |
| **简历可写** | 「建立 ONNX lowering 算子数值对齐回归，覆盖 MatMul/Softmax/Attention/RMSNorm/RoPE 等子图」 |
...
| 项 | 内容 |
|----|------|
| **状态** | 已实现：P5b Stage 4 `pass_nchw_nhwc_layout` / `pass_nhwc_nchw_layout` |
...
| **验证** | `run_golden`（18/18）+ `test_lit_filecheck`（39 项）+ `test_transformer_e2e` |
...
| **验证** | `run_golden`（7/7）+ `test_transformer_e2e`（含 gelu）+ `gelu_legalize.mlir` LIT |
| **简历可写** | 「扩展 Transformer FFN 子图：GELU ONNX→StableHLO（chlo.erf）+ fusion 标注 pass」 |

#### 7.15 Graph Partitioning 教学（P13）✅

| 项 | 内容 |
|----|------|
| **状态** | `16_graph_partition/run_graph_partition.cpp`：单层 Transformer Attention/FFN 切分、边界张量与通信量估算 |
...
| **验证** | `run_golden`（8/8）+ `test_transformer_e2e`（含 swiglu）+ `swiglu_legalize.mlir` LIT |
...
| **简历可写** | 「在 mlir_pass 实现 Q/DQ MatMul 分解子图标注，与 P11 ONNX QDQ 图改写概念对齐」 |
...
| **验证** | `run_golden`（10/10）+ `matmul_bias_fusion.mlir` LIT + shell + `test_transformer_e2e`（含 matmul_bias）+ `test_quant_e2e`（含 P4 qdq） |
...
| **验证** | `run_golden`（11/11）+ LIT + shell + `test_transformer_e2e` |
| **简历可写** | 「实现水平 GEMM 融合标注 pass，覆盖 `concat(matmul(X,W1), matmul(X,W2))` 面试高频模式」 |

### P5 — 已完成（P9 九项补强）

#### 7.21 P11 Q/DQ ONNX → P4 → mlir_pass 全链路 ✅

| 项 | 内容 |
|----|------|
| **状态** | P4 `QuantizeLinear`/`DequantizeLinear` pattern；`check_quant_qdq_matmul`；`run_quant_e2e` 第三条 P11 路径 |
| **验证** | `run_golden` 18/18 + `test_quant_e2e` |
| **简历可写** | 「打通 P11 ONNX Q/DQ → P4 StableHLO → mlir_pass qdq-legalize 跨仓库验证」 |
...
| 项 | 内容 |
|----|------|
| **状态** | `run_jit_golden.py`；`test/jit_scale.mlir`；`test_jit_golden` |
| **验证** | JIT vs NumPy rtol/atol=1e-5 |
| **简历可写** | 「建立 pipe-demo JIT 与 NumPy 数值对齐回归（matmul_add / scale）」 |
...
| **简历可写** | 「mlir_pass 侧对齐 P5b NCHW→NHWC layout 折叠语义的标注 pass」 |
...
| **简历可写** | 「P13 图切分 demo 与 mlir_pass partition 边界 fixture 概念串联」 |
...
#### 7.35 torch-mlir 跨仓库 e2e（A4）✅

| 项 | 内容 |
|----|------|
| **状态** | `run_torch_e2e.sh`：torch-mlir 导出 Conv+BN 或 `conv_bn_torch.mlir` fixture → fusion |
| **验证** | `test_torch_e2e` |
| **简历可写** | 「torch-mlir StableHLO 导出 + mlir_pass fusion 串联（不写 Dynamo）」 |

#### 7.36 FFN JIT golden（A5）✅

| 项 | 内容 |
|----|------|
| **状态** | `gelu_p4_jit.mlir` / `swiglu_p4_jit.mlir` + sigmoid 快速路径；`run_jit_golden.py` **6 项** |
| **验证** | `test_jit_golden` 6/6（GELU P4 `atol=1e-3`） |
| **简历可写** | 「FFN 子图降至 LLVM 后 NumPy 数值对齐」 |
...
#### 7.39 KV decode e2e + P12（B2）✅

| 项 | 内容 |
|----|------|
| **状态** | `lowering_decode_step.onnx`；`run_kvcache_e2e.sh`；`test_kvcache_e2e` |
| **验证** | `test_kvcache_e2e`：fusion + `kvcache_boundary` + P12 decode |
...
| **验证** | `test_partition_smoke` |
| **简历可写** | 「P13 通信量 stdout 与 partition fixture 对齐」 |
...
| Graph Partitioning / 多卡并行 | P13 编译期切分教学；无硬件拓扑与 runtime |
| LLaMA-7B 端到端 + 性能数字 | 无法诚实验证 |
| 真实 GPU kernel 下发 | P10 为 PTX 文本模拟 |
...
| 测试 | 「`mlir_pass` Shell **36** 项 + LIT **39** 项 + **10** 条跨仓库 e2e + JIT golden **6** 项；`mlir_compiler` `run_golden` **18** 项 + `run_calib_to_quant` + CI。工业级缺口见 **§2.2**。」 |
...
| 「P6–P12 都是真实 MLIR Pass」 | 「P6–P12 是教学级 IR 模拟，对应工业 pass 链语义」 |
| 「实现了 LLaMA-7B 全编译」 | 不提 |
...
| F 量化 | P11 Q/DQ、run_quant_qdq | §12 |
| G 测试 | compiler golden 18 + pass JIT golden 6、LIT 39、shell 36、10 条跨仓库 e2e | §15 |
...
| Q/DQ 全链路 | P11→P4→`test_quant_e2e` | **强契合** |
...
| JIT 数值 golden | `test_jit_golden` | 强契合（与 run_golden 组合） |
...
| ONNX→StableHLO + ConversionPattern | P4 tier 3 + `run_lowering` + `run_golden` | 强契合（text IR） |
...
| KVCache memory planning | P12 + `test_kvcache_e2e` | **强契合**（编译期） |
| Linalg/Bufferize/SCF/Vector/LLVM/GPU 梳理 | P6–P10 模拟 | 概念契合 |
...
```bash
cmake -B build && cmake --build build --target run_lowering run_golden run_quant_qdq run_calib_to_quant
...
*最后更新：P10+A1–B6 小步补强 — 28 pass / 36 shell / 39 LIT / JIT golden **6** 项 / compiler golden 18 项 / 10 条 e2e + §2.2 工业级缺口表。*
