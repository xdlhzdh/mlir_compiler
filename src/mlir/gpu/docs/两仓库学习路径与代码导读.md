...
面试时按这三段讲；不要说/推荐说法速查见 [§8 面试表达边界](#8-面试表达边界)。上面提到的 **QDQ** 是全文档最容易被绕晕的概念，两仓库里有 5 条互不相同的代码路径都沾"QDQ"的边，一次性讲清见 [§3.6 QDQ 全景](#36-qdq-全景一次讲清后文不再重复)。上面出现的 **两个"golden"**（compiler 的 18 项、pass 的 6 项）验证的是完全不同的东西，一次性讲清见 [§4.7 Golden 全景](#47-golden-全景三种golden分别验证什么)。
...
| Golden | compiler **18** + pass JIT **6** | `run_lowering_golden.py` 的 `CHECKS`（17）+ `--quant-dir` 的 1 项 = 18；`scripts/run_jit_golden.py` 的 `CASES` = 6；这两种 golden 验证的对象完全不同，见 [§4.7](#47-golden-全景三种golden分别验证什么) |
...
| `quant_models/` | `gen_quant_models.py`（P12） | `quant_qdq_matmul.onnx`，**工业 QDQ 格式**——完整 `QuantizeLinear` + `DequantizeLinear` + MatMul（见 §3.6） | 练 **P12 量化前端**（ONNX QDQ 导入 → qlinear 改写）；被 `run_quant_qdq` 消费，并被 lowering / golden 复用 |
...
> **golden 到底在验证什么，容易被误解**：每个 `check_*` 只做「用 ONNX Runtime 跑原始 `.onnx`」+「用 NumPy 独立手写同一算子的参考公式」两件事再对比，**完全不会**调用 `run_lowering_l1/l2/l3` 或解析它们打印出的 StableHLO 文本——所以 golden 全过，证明的是"我们手写的数学公式和 ONNX 真实语义一致"，只是**间接**给 lowering 正确性背书，并不是直接执行了生成的代码。完整的三种 golden 对比、以及"谁才是真正执行生成代码验证数值"，见 [§4.7 Golden 全景](#47-golden-全景三种golden分别验证什么)。
...
- 能说出 `run_golden` 验证的是"ONNX Runtime vs 手写 NumPy"，从不执行 lowering 生成的 StableHLO，也不是 LLVM JIT；`run_lowering` 与 `run_golden` 是两条独立链路（§4.7）。
...
核心代码用 `partition_hint` 切分，并把两边都使用的 tensor 识别为 boundary tensor。当前 golden comm bytes 是 **262144**（`sync_partition_fixture.py` 里写死的回归基线数字，和 §4.7 讲的 `run_golden`/`test_jit_golden` 不是同一类"golden"，见 [§4.7](#47-golden-全景三种golden分别验证什么)）。
...
| ④ | golden 数值校验 | `run_golden --quant-dir` | `quant_qdq_matmul.onnx` | 用 ONNX Runtime 跑真值，和 NumPy 手算的 quant/dequant 结果对比 | 第 18 项 golden（17 个 lowering CHECKS + 这 1 项） | 否——只验证数值，不改写图 |
...
> 两个 fixture：`lowering_qdq_matmul` 是手搓算式，`quant_qdq_matmul` 才是工业 QDQ 格式。五条路径里只有 P12 的 `run_quant_qdq`（①）在做真正的"QDQ→qlinear"改写；P4 lowering（③）和 `mlir_pass` 的 `qdq-legalize`（⑤）只是把 Q/DQ 语义翻译/标注一遍，golden（④）只管对数值。面试时如果被问「你们的 QDQ 量化是怎么做的」，答案是①，不要把③⑤当成量化本身。
...
### 4.7 Golden 全景：三种"golden"分别验证什么

> 全文档提到"golden"的地方分散在 §1、§2、§3.2、本节和 §5、§7、§9——但字面上都叫 "golden"，背后其实是**互不相干的 3 件事**，混在一起看最容易产生"golden 到底测了什么"的疑惑。本节一次讲清，后文提到 golden 时只给结论并指回这里。

读完本节你应该能一句话说清：**全仓库只有 1 处会真正执行"生成出来的代码"并核对数值（`test_jit_golden`）；`run_golden` 比的是"ONNX Runtime 的真实语义"和"我们手写的 NumPy 公式"，从不执行任何生成的 StableHLO；另外还有一类是写死的回归基线数字（比如 P14 的 comm bytes），和前两者都不是一回事。**

#### 4.7.1 一张表看清 3 种 "golden"

| | ① P4 lowering golden | ② mlir_pass JIT golden | ③ 硬编码回归基线 |
|---|---|---|---|
| **命令 / 脚本** | `run_golden`（= `run_lowering_golden.py`） | `test_jit_golden`（= `run_jit_golden.py` / `.sh`） | 例：`sync_partition_fixture.py`（P14） |
| **所在仓库** | `mlir_compiler` | `mlir_pass` | `mlir_pass`（跨仓库读 compiler 产物） |
| **对比双方** | ONNX Runtime 跑**原始 `.onnx`** vs Python 手写的 NumPy 参考公式 | `pipe-demo --jit` 打印的**真实 LLVM JIT 计算结果** vs Python 手写的 NumPy 参考 | 脚本从 stdout 正则解析出的实际数字 vs 代码里写死的常量 |
| **有没有执行"生成出来的代码"** | **没有**——不 import/subprocess 调用 `run_lowering_l1/l2/l3`，也不解析/执行任何 `.mlir` 文本 | **有**——是全仓库唯一端到端跑通 StableHLO→Linalg→Bufferize→loops→LLVM→`ExecutionEngine` JIT 并核对数值的地方 | 不涉及"数值计算是否正确"，只关心"这个统计量有没有意外变化" |
| **数量** | 17（`CHECKS`）+ 1（`--quant-dir` 的 `quant_qdq_matmul`）= **18** | 6（`CASES`：`matmul_add`/`jit_scale`/`jit_gelu`/`jit_gelu_p4`/`jit_swiglu`/`jit_swiglu_p4`） | 不是"一组 check"，是单个基线值（P14 = **262144**） |
| **验证的是什么** | "我们对 ONNX 算子数学语义的理解，和 ONNX Runtime 真实实现一致" | "真实生成的机器可执行代码，在给定输入下算出的数在数学上是对的" | "某个编译期统计量没有随改动意外漂移"（regression baseline，不是双实现对比） |

> 还有一个更轻量、容易和 ②搞混的检查：`test_shell_regression.sh` 里有一条 `== JIT smoke ==` 场景，只是跑一次 `--jit` 后 `grep '1.5'` 看输出里有没有这个子串，**不做完整数值比对**，也不在 `test_jit_golden` 的 6 个 `CASES` 里——它只是 36 个 shell 场景之一，顺带冒烟一下 JIT 路径没崩。

#### 4.7.2 最容易被误解的一点：`run_golden` 到底有没有跑 lowering 产出的 StableHLO？

**没有。** 直觉上会觉得"golden 校验 lowering 语义正确性"意味着它执行了 lowering 产生的 StableHLO 代码，但翻开 `run_lowering_golden.py` 会发现每个 `check_*` 函数都只做三件事：

1. `onnx.load` 读**原始** `.onnx` 文件的 initializer（权重/scale 等）；
2. 用 NumPy **独立手写**这个算子该怎么算（比如 `check_softmax` 里手写 `_softmax(x)`）；
3. 用 `onnxruntime.InferenceSession` 跑这个 `.onnx`，把结果和第 2 步的 NumPy 结果 `np.allclose` 比较。

全程**不 import、不 subprocess 调用** `run_lowering_l1/l2/l3`，也**不解析**它们打印出的 StableHLO 文本。所以准确的说法是：

- `run_golden` 全部 PASS，证明的是"NumPy 里那份参考公式，和 ONNX Runtime 真实跑这个算子的结果一致"——这份参考公式和 lowering 转换代码（如 `convert_matmul`、`DequantizeLinearOpConversion`）里用的是**同一套数学**，所以能给"lowering 语义写对了"提供强有力的**间接**证据。
- 但如果 `emit_dot_general`/`emit_convolution` 这些 C++ 发射函数本身有 bug（哪怕只是维度写错，只要还能被 `mlir-opt` 解析成合法 IR），`run_golden` **也不会**发现，因为它压根没跑到那段代码，也没有解析生成的 `.mlir` 文本做结构检查。
- 想要"真的跑一遍生成/降低后的代码、核对数值"，要看 mlir_pass 侧的 ②`test_jit_golden`——那是全仓库唯一把"StableHLO 文本 → 真实机器码 → 执行 → 拿到数字"这条链路走完并核对数值的地方。

一句话：**`run_golden` 验证的是"我们理解的算子数学对不对"，`test_jit_golden` 验证的是"编译出来的代码算得对不对"，两者验证的对象完全不同，不能互相替代。**

#### 4.7.3 一句话记忆

> ①`run_golden`（compiler 侧，18 项）比的是"ONNX Runtime vs 手写 NumPy"，只验证对 ONNX 算子数学语义的理解，从不执行任何生成的 StableHLO/LLVM 代码。②`test_jit_golden`（pass 侧，6 项）比的是"真实 LLVM JIT 执行结果 vs 手写 NumPy"，是全仓库唯一端到端跑生成代码验证数值的地方。③像"P14 golden comm bytes = 262144"这种，只是一个写死的回归基线数字，和①②都不是一回事。
...
| `run_jit_golden.sh` | `test_jit_golden` | LLVM JIT vs NumPy，全仓库唯一真正执行生成代码验证数值的地方（§4.7） |
...
1. `run_golden` 和 `test_jit_golden` 分别验证什么？（§4.7：谁执行了生成代码，谁只对了个数学公式）
