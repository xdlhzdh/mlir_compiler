...
- [`mlir_compiler/README.md`](../../../../README.md)：P1–P13 教学 pipeline。
...
| 仓库 | 角色 | 重点学习内容 | 实现形态 |
|------|------|--------------|----------|
| `mlir_compiler` | AI 编译器教学地图 | ONNX 前端、StableHLO lowering、golden、量化、内存、图分区 | P1–P4/P5a 偏真实；P6–P13 多为 C++ header-only 教学 IR |
...
flowchart TB
  ...
  P11[compiler P11 Quant] --> P4
  P12[compiler P12 KV memory] -. concept .-> FUS
  P13[compiler P13 partition] -. fixture .-> FUS
```
...
| 5 | P11 quant | P12 KV | P13 partition | plugin | 全量验证 | 对照能力映射自讲 |
...
> 速成可**跳过**：§3.3–§3.5（P11/P12/P13 教学模拟）、§4.5 中后端细节、§6 全部补课点。这些标了「可选/深挖」。
...
> **核心 / 可选标记**：§3.1（P1–P3）、§3.2（P4）是**核心主干**，所有路线都要读；§3.3–§3.5（P11/P12/P13）是**教学模拟**，速成可跳过、标准第 5 周再看、深度必读。
>
...
验证（`run_graph` 是 **P1–P3 的一键链路**：按 `run_onnx_parse` → `run_onnx_to_ir` → `run_graph_rewrite` 顺序依次跑，编排见 `common/run_graph.cmake.in`。**别和 P13 的 `run_graph_partition_demo` 混淆**——那是图切分，见 §3.5）：
...
| 文件 | 核心代码描述 |
|------|--------------|
| `5_onnx_to_stablehlo/run_level1.cpp` | **Tier 1**：手写 5 种基础 op lowering（`dispatch` + `convert_*`） |
| `5_onnx_to_stablehlo/run_level2.cpp` | **Tier 2**：broadcast、动态 shape、完整 Conv 属性 |
| `5_onnx_to_stablehlo/run_level3.cpp` | **Tier 3**：20+ 种 ONNX op pattern + `--mlir-only` 导出 |
| `5_onnx_to_stablehlo/conversion_framework.h` | 教学版 DialectConversion：`ConversionPattern`、`ConversionTarget`、`RewritePatternSet`、`applyFullConversion` |
| `5_onnx_to_stablehlo/run_lowering.cmake.in` | `run_lowering` 的 tier 1→2→3 执行编排 |
| `5_onnx_to_stablehlo/gen_lowering_models.py` | 生成全部 `lowering_*.onnx` fixture（tier 1/2/3 共用） |
| `5_onnx_to_stablehlo/stablehlo_ir.h` | StableHLO 文本发射器 |
| `5_onnx_to_stablehlo/run_lowering_golden.py` | ONNX Runtime vs NumPy golden，18 项 |
...
| 目录 | 生成脚本 | 内容 | 目的 / 谁消费 |
|------|----------|------|---------------|
| `lowering_models/` | `gen_lowering_models.py`（P4） | tier 1/2/3 通用 lowering fixture（含一个**概念级** `lowering_qdq_matmul.onnx`：只有 dequant 链 + MatMul，用来*模拟*量化图形态） | 练 **ONNX→StableHLO lowering**；被 `run_lowering_l1/l2/l3` + `run_golden` 的 17 项 CHECKS 消费 |
| `quant_models/` | `gen_quant_models.py`（P11） | `quant_qdq_matmul.onnx`：**完整** QuantizeLinear + DequantizeLinear + MatMul（真 Q/DQ 节点对） | 练 **P11 量化前端**（ONNX QDQ 导入 → qlinear 改写）；被 `run_quant_qdq` 消费，并被 lowering / golden 复用 |

关键区别：**lowering_models 面向「把各种算子降到 StableHLO」**（覆盖 Transformer/broadcast/dynamic 等），里面的 `lowering_qdq_matmul` 只是用 dequant 链**模拟**量化图、不含真正的 `QuantizeLinear`；**quant_models 面向「量化前端」**，`quant_qdq_matmul.onnx` 含完整 Q/DQ 节点对，给 `run_quantization` 做导入 + 改写。
...
# 或直接调脚本：
python3 src/mlir/gpu/5_onnx_to_stablehlo/gen_lowering_models.py build/src/mlir/gpu/lowering_models
python3 src/mlir/gpu/14_quantization/gen_quant_models.py        build/src/mlir/gpu/quant_models
```
...
**（2）为什么会有它 / 解决什么问题？** 真实量化工具（ONNX Runtime quantizer、TensorRT 等）导出量化模型时，**不会直接给你 int8 kernel**，而是用 Q/DQ 节点对**标注**「这个张量应该按 int8 存、scale/zp 是多少」。这就是工业界的 **QDQ 格式**。编译器（P11）的任务，就是**识别这些 Q/DQ 标注，把它们折叠成一个真正的量化算子**（`qlinear_matmul`）。所以这个 fixture 存在的意义是：**练「编译器如何吃进一张 QDQ 图、改写成量化推理图」**——这是量化前端的核心工作，和普通 ONNX 无关。
...
|--------|------|--------------------------------------|------|
| **① P11 量化前端（主用途，才是重点）** | `run_quant_qdq` | `run_quantization --onnx …`：`onnx_qdq_import.cpp` 把 QDQ 图导入内部 `quant_ir`（识别 3 Q + 3 DQ + MatMul），再 `stage4_rewrite_imported_qdq` 把 `DQ→MatMul→DQ` **折叠成一个 `qlinear_matmul`** | 量化推理图（`quant_ir`，**不是** StableHLO） |
...
- **量化改写（真正的量化工作）不在 lowering 里**——它在 ① `run_quant_qdq`，产物是 `qlinear_matmul`，走的是 `quant_ir` 而不是 StableHLO。
...
> 一句话记忆：**`quant_qdq_matmul.onnx` 是给 P11「量化前端」练手的（QDQ→qlinear），lowering 只是顺带证明框架能识别 Q/DQ；真正的量化改写和 lowering 是两条线。**
...
如果带 `--quant-dir`，还会额外跑 P11 `quant_qdq_matmul`，因此总数是 18。
...
`14_quantization/run_quantization.cpp` 是一个 7 步教学 pipeline：
...
### 3.4 P12：内存规划与 KVCache

`15_memory_planning/run_memory_planning.cpp` 用教学 IR 模拟 buffer lifetime：
...
`16_graph_partition/run_graph_partition.cpp` 把单层 Transformer 分成 Attention partition 和 FFN partition：
...
| `run_quant_e2e.sh` | `test_quant_e2e` | 手写 Q/DQ + P4 Q/DQ + P11 Q/DQ |
...
| `run_partition_smoke.sh` | `test_partition_smoke` | P13 partition + comm golden |
...
| §6.6 NCCL / Tensor·Pipeline Parallel | **P2** | "P13 是真多卡 runtime 吗？" | 说清"编译期切分 vs 通信 runtime" |
...
项目边界：P13 是编译期 partition planning，没有 collective runtime。
...
项目边界：P12/KV pass 只做编译期内存规划和 `kvcache_boundary` 标注。
...
| 实现了 KVCache runtime | P12 编译期内存规划模拟 + `kvcache_boundary` 标注 |
| P6–P12 都是真实 MLIR Pass | `mlir_compiler` P6–P13 多为教学 IR；`mlir_pass` 是真实 MLIR |
| INT8 性能提升 XX% | Q/DQ 图改写与混合精度教学分析，无真实性能数字 |
| 多卡分布式 runtime | P13 编译期图切分和通信量估算，无 NCCL runtime |
...
7. P11 Q/DQ 和 `qdq-legalize` 的边界在哪里？
8. P12 KVCache 模拟与真正 runtime cache manager 有什么差别？
9. P13 comm bytes 为什么是 boundary tensor 估算，不是 NCCL 实测？
