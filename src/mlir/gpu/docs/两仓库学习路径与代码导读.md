...
> `mlir_compiler` 负责前端：把 ONNX 模型解析并降到 StableHLO（P4 三级 lowering），用 ONNX Runtime vs NumPy 做 18 项 golden 校验 lowering 语义正确性。
>
> `mlir_pass` 负责中后端：接手导出的 StableHLO `.mlir`，在真实 MLIR PassManager 中做 Conv+BN 等 7 个真图变换、Transformer/FFN/GEMM/QDQ/Layout/KV 等 13 个子图标注，再经 Linalg → Bufferize → SCF/Affine/Vector 降到 LLVM Dialect，用 JIT golden 验证数值。
>
...
面试时按这三段讲；不要说/推荐说法速查见 [§8 面试表达边界](#8-面试表达边界)。
...
不要反过来学。`mlir_pass` 的输入已经是 StableHLO `.mlir`，如果没有先理解 `mlir_compiler` 的 P4 lowering、golden 和 ONNX fixture，很难判断 pass 在优化什么。
...
| Golden | compiler **18** + pass JIT **6** | `run_lowering_golden.py` 的 `CHECKS`（17）+ `--quant-dir` 的 1 项 = 18；`scripts/run_jit_golden.py` 的 `CASES` = 6 |
| Transformer e2e fixture | **11** | `scripts/run_transformer_e2e.sh` 里 `run_case` 调用数 = 11 |
| 跨仓库 e2e 脚本 | **10** | `test/CMakeLists.txt` 里 `test_*_e2e` / `_smoke` / `_golden` 的 target 数 = 10 |

### 1.3 两仓库分工

| 仓库 | 角色 | 重点学习内容 | 实现形态 |
|------|------|--------------|----------|
| `mlir_compiler` | AI 编译器教学地图 | ONNX 前端、StableHLO lowering、golden、量化、内存、图分区 | P1–P4 偏真实（Protobuf + StableHLO 文本）；P5–P14 为 C++ header-only 教学 IR |
...
| 2 | `mlir_compiler` | P4 ONNX→StableHLO + golden | 能读 `ConversionPattern`，`run_golden` 18/18 |
| 3 | `mlir_pass` | fusion stage + PatternRewrite | 能解释 20 个 fusion pass 顺序 + 真变换/标注之分，跑通 `test_transformer_e2e` |
| 4 | `mlir_pass` | Linalg → Bufferize → SCF/Affine/Vector → LLVM/JIT | 能对比四条 `--loop-mode`，跑通 JIT golden |
...
| 2 | §3.2 tier 1/2/3 区别 | `run_level1` 手写 lowering | `run_level2` broadcast/dynamic | `run_level3` + framework | golden | 导出 2 个 `.mlir` |
| 3 | `PipelineStages.cpp` | `ConvBNFusion.cpp` | `SoftmaxLegalize.cpp` | transformer e2e | quant/layout e2e | INTERVIEW §14 |
| 4 | bufferize | linalg | SCF | affine | vector | JIT golden |
...
4. 跑通 **1 条 e2e + JIT**：`ninja test_transformer_e2e test_jit_golden`。
5. 背熟 **§8 面试边界** 和一分钟故事。

> 速成可**跳过**：§3.3–§3.5（P12/P13/P14 教学模拟）、§4.5 中后端细节、§6 全部补课点。这些标了「可选/深挖」。

**深度（8–10 周）——在标准路线之上动手改代码**

先完成标准 5 周计划，再从 §10「建议深挖任务」里挑 2–3 个真正落地（每个都要有验收产物）：

1. 新增一个 ONNX op pattern（改 `mlir_compiler` P4）→ `run_golden` 多一项通过。
...
> **核心 / 可选标记**：§3.1（P1–P3）、§3.2（P4）是**核心主干**，所有路线都要读；§3.3–§3.5（P12/P13/P14）是**教学模拟**，速成可跳过、标准第 5 周再看、深度必读。
>
...
**P12 QDQ 相关的完整输入/输出链（最容易混淆）：**

```text
┌─ 路径 A（P12 默认，教学量化流程）─────────────────────────────────────┐
│  输入：run_quantization 内部 stage0_setup() 硬编码的 Conv+BN+ReLU+MatMul 图
│  过程：7 步量化（校准→scale→fusion→rewrite→mixed→summary）
│  输出：stdout 打印 quant_ir::Graph（含 qlinear_conv/qlinear_matmul）
│  终点：不 lower 到 StableHLO，不产生 .mlir 文件
└──────────────────────────────────────────────────────────────────────┘

┌─ 路径 B（run_quant_qdq，P12 QDQ 导入路径）─────────────────────────────┐
│  输入：quant_models/quant_qdq_matmul.onnx（工业 QDQ 格式，有 Q/DQ 节点）
│  过程：load_onnx_qdq → stage4_rewrite_imported_qdq（DQ→MatMul→DQ 折叠）
│  输出：stdout 打印改写后的 quant_ir::Graph（含 qlinear_matmul）
│  终点：不 lower 到 StableHLO，不产生 .mlir 文件
└──────────────────────────────────────────────────────────────────────┘

┌─ 路径 C（run_lowering L3 顺带处理 QDQ）────────────────────────────────┐
│  输入：quant_models/quant_qdq_matmul.onnx（同一个文件）
│  过程：L3 tier3 框架，Q 直通、DQ 展开成 (q-zp)*scale 的 shlo 算子
│  输出：stdout 打印 shlo::ModuleOp（纯 float StableHLO 文本，不是量化图）
│  终点：与路径 B 完全不同——这里只是验证 tier3 能识别 Q/DQ op 类型
└──────────────────────────────────────────────────────────────────────┘
```

路径 B 和路径 C 消费同一个 `.onnx` 文件，但进入**完全不同的代码路径**，产出**完全不同格式的 IR**，不可混淆。
...
| `4_onnx_to_stablehlo/run_lowering_golden.py` | ONNX Runtime vs NumPy golden，18 项 |
...
| 目录 | 生成脚本 | 内容 | 目的 / 谁消费 |
|------|----------|------|---------------|
| `lowering_models/` | `gen_lowering_models.py`（P4） | tier 1/2/3 通用 lowering fixture（含 `lowering_qdq_matmul.onnx`：**不是**工业 QDQ，见下节） | 练 **ONNX→StableHLO lowering**；被 `run_lowering_l1/l2/l3` + `run_golden` 的 17 项 CHECKS 消费 |
| `quant_models/` | `gen_quant_models.py`（P12） | `quant_qdq_matmul.onnx`：**工业 QDQ 格式**——完整 `QuantizeLinear` + `DequantizeLinear` + MatMul | 练 **P12 量化前端**（ONNX QDQ 导入 → qlinear 改写）；被 `run_quant_qdq` 消费，并被 lowering / golden 复用 |

关键区别：**lowering_models 面向「把各种算子降到 StableHLO」**（覆盖 Transformer/broadcast/dynamic 等）；**quant_models 面向「量化前端」**，`quant_qdq_matmul.onnx` 才是工业界 QDQ 格式。

> **格式说明**：两个目录下的 `*.onnx` 都是标准 ONNX protobuf 计算图（`ModelProto` → `GraphProto`），由 `onnx.helper` 脚本生成，**不是** PyTorch/TF 导出的训练图；其中 `lowering_qdq_matmul` 甚至连 Q/DQ 标准算子都没有。

##### `lowering_qdq_matmul` vs `quant_qdq_matmul`：名字像，本质完全不同

很多人看到两个文件名都带 `qdq_matmul` 就以为是一回事——**不是**。对比如下：

| | `lowering_qdq_matmul.onnx`（P4） | `quant_qdq_matmul.onnx`（P12） |
|---|----------------------------------|-------------------------------|
| **所在目录** | `lowering_models/` | `quant_models/` |
| **是不是工业 QDQ** | **否** | **是** |
| **图里有什么** | 普通 `Sub` + `Mul` 手搓反量化链 + `MatMul` | 标准 `QuantizeLinear` / `DequantizeLinear` + `MatMul` + scale/zp |
| **有没有 int8 张量** | 无（全程 float） | 有（`qx`/`qw`/`qy` 为 int8） |
| **工业工具能认吗** | ORT 能跑，但**不是** QDQ 标注格式 | ORT quantizer / TensorRT 导出的就是这种格式 |
| **主要消费者** | `run_lowering_l3` + `run_golden`（17 项之一） | `run_quant_qdq`（量化前端主用途） |

**`lowering_qdq_matmul` 的图结构**（`gen_lowering_models.py` 手写，模拟 dequant 语义）：

```text
X(f32) ── Sub(zp_x) ── Mul(scale_x) ── dq_x(f32) ┐
                                                  ├─ MatMul ── Y(f32)
W(f32) ── Sub(zp_w) ── Mul(scale_w) ── dq_w(f32) ┘
```

**`quant_qdq_matmul` 的图结构**（工业 QDQ，见 `gen_quant_models.py`）：

```text
X(f32) → QuantizeLinear → qx(int8) → DequantizeLinear → dq_x(f32) ┐
                                                                    ├─ MatMul → … → Y(f32)
W(f32) → QuantizeLinear → qw(int8) → DequantizeLinear → dq_w(f32) ┘
```

所以：`lowering_qdq_matmul` 只是 P4 用来**练 lowering 时遇到「带 scale/zp 的 dequant 链」这种图形态**的教学 fixture，**不能**拿去练 P12 的 QDQ 导入与 qlinear 折叠；那件事要用 `quant_qdq_matmul`。
...
每个二进制：读 ONNX → 逐节点 lowering → 打印 StableHLO MLIR 到 stdout（不另存文件）。**数值语义校验走独立的 `run_golden`**（ONNX Runtime vs NumPy，不调用 L1/L2/L3）——即 `run_lowering`（看结构）与 `run_golden`（看数值）是两条独立链路。

##### 深入：`quant_qdq_matmul.onnx` 到底是什么、要不要 lowering

这是最容易一头雾水的地方，拆开讲。

**（1）它长什么样？** 它是一张 **QDQ 格式**的 ONNX 图（Quantize-Dequantize）。一个普通 `X @ W` 矩阵乘，被在每个张量前后插入了 `QuantizeLinear`（Q）/`DequantizeLinear`（DQ）成对节点，外加 `scale`/`zero_point` 参数：

```text
X(f32) → Q → qx(int8) → DQ → dq_x(f32) ┐
                                        ├─ MatMul → mm(f32) → Q → qy(int8) → DQ → Y(f32)
W(f32) → Q → qw(int8) → DQ → dq_w(f32) ┘
```

即 **3 对 Q/DQ + 1 个 MatMul**。对比 `lowering_basic.onnx` 那种「纯 float 算子图」，它多了量化语义。

**（2）为什么会有它 / 解决什么问题？** 真实量化工具（ONNX Runtime quantizer、TensorRT 等）导出量化模型时，**不会直接给你 int8 kernel**，而是用 Q/DQ 节点对**标注**「这个张量应该按 int8 存、scale/zp 是多少」。这就是工业界的 **QDQ 格式**。编译器（P12）的任务，就是**识别这些 Q/DQ 标注，把它们折叠成一个真正的量化算子**（`qlinear_matmul`）。所以这个 fixture 存在的意义是：**练「编译器如何吃进一张 QDQ 图、改写成量化推理图」**——这是量化前端的核心工作，和普通 ONNX 无关。

**（3）它和普通 ONNX 的区别？**

| | 普通 ONNX（如 `lowering_basic`） | QDQ ONNX（`quant_qdq_matmul`） |
|---|----------------------------------|-------------------------------|
| 图内容 | 纯 float 算子（Add/MatMul…） | float 算子 + `QuantizeLinear`/`DequantizeLinear` + scale/zp |
| 语义 | 只有计算 | 额外携带「哪些张量走 int8、精度参数多少」的**量化计划** |
| 数值 | 就是计算结果 | Q 再 DQ 近似恒等（有量化舍入误差），结构上标注了 int8 意图 |

**（4）它要不要 lowering？—— 关键：有两拨完全不同的代码在处理它，别混为一谈。**

| 消费者 | 命令 | 对 `quant_qdq_matmul.onnx` 做了什么 | 产物 |
|--------|------|--------------------------------------|------|
| **① P12 量化前端（主用途，才是重点）** | `run_quant_qdq` | `run_quantization --onnx …`：`onnx_qdq_import.cpp` 把 QDQ 图导入内部 `quant_ir`（识别 3 Q + 3 DQ + MatMul），再 `stage4_rewrite_imported_qdq` 把 `DQ→MatMul→DQ` **折叠成一个 `qlinear_matmul`** | `quant_ir::Graph`（**自定义结构体，不是 MLIR，不需要 lower 到 StableHLO，打印 stdout 后结束**） |
| **② L3 lowering（次要复用）** | `run_lowering`（末步） | 把它当**普通 ONNX** 走 tier 3。但 Q/DQ 的 pattern 是**教学简化**：`QuantizeLinear` = **直通**（保留 float SSA，并不真的量化成 int8）；`DequantizeLinear` = 发 `(q - zp) * scale` 的 `stablehlo.subtract + multiply` | 一张**纯 float** 的 StableHLO 图 |
| **③ golden 校验** | `run_golden --quant-dir` | 用 ONNX Runtime 跑这张图，和 NumPy 手算的 quant/dequant 结果对比 | 第 18 项 golden（`17 + 1`） |

**所以「要不要 lowering」的答案是：**

- **量化改写（真正的量化工作）不在 lowering 里**——它在 ① `run_quant_qdq`，产物是 `quant_ir::Graph`（自定义结构体，`namespace quant_ir`，纯 C++17，无 MLIR 依赖），不是 StableHLO，**也不需要再 lower 到 StableHLO**；整个 P12 流程打印 stdout 后即结束。
- **lowering 也确实会碰它（②），但只是「顺带验证 tier 3 框架能识别 Q/DQ 这两种 op 类型」**，而且是教学简化：Q 当直通、DQ 当仿射 `(q-zp)*scale`，**产出的是 float 图，不是 int8 kernel**。

**（5）L3 lowering 它 和 lowering 普通 ONNX 有什么区别？**

- 普通 op（Add/MatMul）：把 float 算子翻译成 StableHLO **float 算子**。
- QDQ 里的 Q/DQ：`Q` 直接透传（教学上不产生 int8），`DQ` 展开成 `subtract + multiply` 的 float 反量化公式。**本质仍是「翻译成 float StableHLO」，只是多了 Q/DQ 两种 op 的处理规则**；它**没有**做「int8 kernel 生成」这种工业级量化 lowering。

命令：

```bash
cmake --build build --target gen_quant_models run_quant_qdq   # ① 量化前端：QDQ 导入 + qlinear 改写（重点）
cmake --build build --target run_lowering                     # ② 末步顺带把它降成 float StableHLO
cmake --build build --target run_golden                       # ③ 内置 --quant-dir，第 18 项 quant golden
```

> 一句话记忆：**`quant_qdq_matmul.onnx` 是给 P12「量化前端」练手的（QDQ→qlinear），lowering 只是顺带证明框架能识别 Q/DQ；真正的量化改写和 lowering 是两条线。**
...
>
> 所以我理解的「会写 lowering」不是背 op 对照表，而是能把算子的数学语义和框架间的表示差异正确翻译过去，并且能验证——我用 `run_golden` 拿 ONNX Runtime 对比 NumPy 确认 lowering 语义正确。
...
| 怎么保证 lowering 是对的？ | `run_golden`：同一个 ONNX 用 ONNX Runtime 跑参考值，和 lowering 后的语义对比（rtol/atol） |
...
golden 框架的核心（`run_lowering_golden.py`）：
...
- 能说出 `run_golden` 验证的是 ONNX→StableHLO lowering 的语义，不是 LLVM JIT；`run_lowering` 与 `run_golden` 是两条独立链路。
- 能手工 trace `lowering_matmul_softmax.onnx` 为什么最后不应残留 `stablehlo.subtract`。

### 3.3 P12：量化教学链路

`12_quantization/run_quantization.cpp` 是一个 7 步教学 pipeline：

```cpp
// Stage 1: Calibration       — collect min/max/histogram
// Stage 2: Scale Computation — compute scale + zero_point
// Stage 3: Operator Fusion   — Conv+BN+ReLU -> QConv
// Stage 4: Graph Rewrite     — insert quantize/dequantize ops
// Stage 5: Mixed Precision   — choose FP16 vs INT8
```

这个工程已经覆盖 Q/DQ 图改写概念，但没有做工业级「校准 JSON 驱动真实 INT8 kernel」。
...
核心代码用 `partition_hint` 切分，并把两边都使用的 tensor 识别为 boundary tensor。当前 golden comm bytes 是 **262144**。
...
| `qdq-legalize` | `qdq_*` | 标 Q/DQ 结构，非真 INT8 kernel |
...
| `run_quant_e2e.sh` | `test_quant_e2e` | 手写 Q/DQ + P4 Q/DQ + P12 Q/DQ |
...
| `run_partition_smoke.sh` | `test_partition_smoke` | P14 partition + comm golden |
| `run_jit_golden.sh` | `test_jit_golden` | LLVM JIT vs NumPy |
...
> 验证上我区分两条链路：`run_lowering` 看结构转换是否正确，`run_golden` 用 ONNX Runtime 对比 NumPy 做语义数值校验。  
...
1. `run_golden` 和 `test_jit_golden` 分别验证什么？
...
7. P12 Q/DQ 和 `qdq-legalize` 的边界在哪里？
8. P13 KVCache 模拟与真正 runtime cache manager 有什么差别？
9. P14 comm bytes 为什么是 boundary tensor 估算，不是 NCCL 实测？
...
| 任务 | 目标 | 验收 |
|------|------|------|
| 新增一个 ONNX op pattern | 学 P4 conversion | `run_golden` 新增 1 项 |
...
*文档版本：AI 编译器学习唯一主文档（28 pass = fusion 20 + 中后端 8 / 36 shell / 39 LIT / compiler golden 18 / JIT golden 6 / 10 条跨仓库 e2e）。所有指标均从代码/脚本核对，复算方法见 §1.2「来源」列。*
