...
与 **1.3 相同安装前缀**，形成「带 StableHLO 的一套 LLVM/MLIR」。**StableHLO 用于 `gpu/` 的 L2（P5）等目标**；本目录 **matmul 流水线不经过 StableHLO**，但共用同一套 MLIR 安装前缀（`simple_compiler` 的 **gpu** 目标亦依赖此布局）。
...
**语义**：**bufferize（缓冲化）** 主要是把 IR 里的 **`tensor`（值语义）改成 `memref`（显式内存块）**，并插入/整理 **分配与释放**（`-buffer-deallocation-pipeline` 与生命周期相关）。在分层上它是 **L3 内的类型落地 Pass（OSB）**（见 **[../README.md](../README.md)**）：核心是先决定 **缓冲区如何落地、哪些结果可原地更新、哪些必须新分配**；**产物仍是 L3 结构化算子**（`linalg` on `memref`），**不是**进入 L4 的标志——须经 **`-convert-linalg-to-loops`** 才生成显式循环。**不是**「把整个 Linalg 方言删掉、换成一个叫 memref 的方言」：`memref` 是 **类型/内存抽象**，算子仍可能是 **`linalg` on `memref`**，或后续再被别的 pass 拆成 `memref.load`/`store` + 循环等。
