...
- **BNF**：**Backus–Naur Form**（巴科斯-诺尔范式），用产生式 `A → α` 来定义文法。
- **EBNF**：**Extended Backus–Naur Form**（扩展巴科斯-诺尔范式），在 BNF 的基础上增加一些“语法糖”，更贴近代码/手写规则，例如：
  - **可选**：`[ X ]`（要么出现 X，要么不出现）
  - **重复**：`{ X }`（X 重复 0 次或多次）
  - **分组**：`( ... )`
  - **选择**：`A | B`

### 1.2 从 EBNF 转成 BNF 的原理（核心思想）

**一句话**：BNF 里只有「产生式 + 选择 + ε（空串）」；所以把 EBNF 里的 `[]`、`{}` 这类结构，统一改写成 **引入辅助非终结符** + **递归** + **ε 分支**。

常见改写模板（记 `X` 为一个符号/符号串）：

- **可选**：`[ X ]`
  - 改写：`OptX → X | ε`
- **重复 0+ 次**：`{ X }`
  - 改写（右递归尾巴，更适合 LL 分析）：`TailX → X TailX | ε`
- **重复 1+ 次**：`X { X }`
  - 改写：`OnePlusX → X TailX`，其中 `TailX → X TailX | ε`
- **带分隔符的列表**：`[ X { "," X } ]`
  - 常见 BNF 写法：`List → X ListTail | ε`，`ListTail → "," X ListTail | ε`

> 这也是为什么在“纯 BNF”里你会看到很多 `Something'`：它们本质上就是把 EBNF 的 `{...}` / `[...]` 展开成“尾部递归 + ε”的辅助符号，方便算 FIRST/FOLLOW、检查 LL(1) 冲突。
